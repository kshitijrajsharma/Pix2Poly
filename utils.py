...
def save_checkpoint(
    state, folder="logs/checkpoint/run1", filename="my_checkpoint.pth.tar"
):
...
def test_generate(model, x, tokenizer, max_len=50, top_k=0, top_p=1):
    x = x.to(CFG.DEVICE)
    batch_preds = (
        torch.ones((x.size(0), 1), device=CFG.DEVICE).fill_(tokenizer.BOS_code).long()
    )
    confs = []

    if top_k != 0 or top_p != 1:
        sample = (
            lambda preds: torch.softmax(preds, dim=-1)
    ...
    with torch.no_grad():
        for i in range(max_len):
            ...
            preds = top_k_top_p_filtering(
                preds, top_k=top_k, top_p=top_p
            )  # if top_k and top_p are set to default, this line does nothing.
            if i % 2 == 0:
                confs_ = (
        ...
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            perm_preds = model.module.scorenet1(feats) + torch.transpose(
                model.module.scorenet2(feats), 1, 2
            )
        else:
            perm_preds = model.scorenet1(feats) + torch.transpose(
                model.scorenet2(feats), 1, 2
            )
...
def save_single_predictions_as_images(
    ...
    with torch.no_grad():
        ...
        )  # list of polygon coordinate tensors
    ...
    for b in range(B):
        vertex_pred_vis[b] = torchvision.utils.draw_segmentation_masks(
            (x[b] * 255).to(dtype=torch.uint8), torch.zeros_like(x[b, 0]).bool()
        )
    ...
    torchvision.utils.save_image(
        vertex_pred_vis.float() / 255,
        f"{folder}/corners_mask/corners_mask_{b}_{epoch}.png",
    )
    ...
    for b in range(B):
        for c in range(len(batch_polygons[b])):
            ...
            if len(cnt) > 0:
                cv2.fillPoly(polygons[b, 0], pts=[cnt], color=1.0)
    ...
    for b in range(B):
        poly_out[b] = torchvision.utils.draw_segmentation_masks(
            (x[b] * 255).to(dtype=torch.uint8), polygons[b, 0].bool()
        )
    ...
    for b in range(B):
        ymask_out[b] = torchvision.utils.draw_segmentation_masks(
            (x[b] * 255).to(dtype=torch.uint8), y_mask[b, 0].bool()
        )
    ...
    torchvision.utils.save_image(ymask_out / 255.0, f"{folder}/gt_mask_{idx}.png")
    torchvision.utils.save_image(y_corner_mask * 255, f"{folder}/gt_corners_{idx}.png")
    torchvision.utils.save_image(
        y_perm[:, None, :, :] * 255, f"{folder}/gt_perm_matrix_{idx}.png"
    )
...
def center_crop_resize(img: Image.Image, output_size: tuple) -> Image.Image:
    """
    Crops the image to a center square, then resizes to output_size.
    ...
    Returns:
        Cropped and resized PIL Image object.
    """
