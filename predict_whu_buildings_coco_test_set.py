...
def main():
    ...
    with torch.no_grad():
        ...
        for i_batch, (x, y_mask, y_corner_mask, y, y_perm, idx) in enumerate(
            ...
            plt.savefig(
                f"runs/{EXPERIMENT_NAME}/test_preds/{DATASET}/{PART_DESC}/{ckpt_desc}/batch_{i_batch}.png"
            )
            plt.close()
    ...
    with open(
        f"runs/{EXPERIMENT_NAME}/test_metrics_{DATASET}_{PART_DESC}_{ckpt_desc}.txt",
        "w",
    ) as ff:
